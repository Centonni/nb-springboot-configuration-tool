 ...
 *
 * The entry point of completion support. 
...
public abstract class AbstractSpringBootConfigurationCompletionProvider implements CompletionProvider {
    ...
    public CompletionTask createTask(int queryType, JTextComponent jtc) {
        ...
        final Set<ConfigurationMetadata> configurationMetas = new HashSet<ConfigurationMetadata>(configurationMetaFiles.size());
            ...
            protected void query(CompletionResultSet completionResultSet, Document document, int caretOffset) {
                ...
                for (ConfigurationMetadata configurationMeta : configurationMetas) {
                    for (ItemMetadata item : configurationMeta.getItems()) {
                        if (item.isOfItemType(ItemMetadata.ItemType.PROPERTY) && !item.getName().equals("") && item.getName().contains(filter)) {
                            completionResultSet.addItem(getCompletionItem(item, cp, startOffset, caretOffset));
                        }
                    }
                }
    ...
    public abstract CompletionItem getCompletionItem(ItemMetadata configurationItem, ClassPath classPath, int dotOffset, int caretOffset);
